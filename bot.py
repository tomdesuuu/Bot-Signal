...
# =====================
# NUMPY/PANDAS WORKAROUND - NO REINSTALL NEEDED
# =====================
...
try:
    import numpy as np
    # Patch numpy to avoid version conflicts
...
try:
    import pandas as pd
    # Patch pandas to avoid version conflicts
...
class TradingModel:
    def __init__(self, symbol: str, timeframe: str, version=Config.MODEL_VERSION):
        ...
        self.model_key = f"{symbol}_{timeframe}_{version}"
...
class TradingBot:
    def __init__(self):
        ...
        # Initialize Telegram with increased timeout
        self.bot = None
        try:
            self.bot = Bot(
            ...
            self.bot.get_me()  # Test connection
            logger.info("Telegram connected successfully")
        except Exception as e:
            logger.error(f"Telegram connection failed: {e}")
            sentry_sdk.capture_exception(e)
        
        # Initialize Telegram Updater for command handling
        try:
            ...
            self.dispatcher.add_handler(CommandHandler('status', self.telegram_status))
            self.dispatcher.add_handler(CommandHandler('signals', self.telegram_active_signals))
            self.dispatcher.add_handler(CommandHandler('cancel', self.telegram_cancel))
            
            # Start polling in a separate thread
            self.updater.start_polling()
            logger.info("Telegram command handlers initialized")
        except Exception as e:
            logger.error(f"Telegram command setup failed: {e}")
            sentry_sdk.capture_exception(e)
    ...
    # =====================
    def telegram_status(self, update: Update, context: CallbackContext):
        ...
        try:
            ...
            update.message.reply_text("⚠️ Error fetching status")
        finally:
            Session.remove()
    
    def telegram_active_signals(self, update: Update, context: CallbackContext):
        ...
        try:
            ...
            if not signals:
                update.message.reply_text("ℹ️ No active signals")
                return
            ...
            update.message.reply_text("⚠️ Error fetching signals")
        finally:
            Session.remove()
    
    def telegram_cancel(self, update: Update, context: CallbackContext):
        ...
        if not context.args:
            update.message.reply_text("Usage: /cancel <signal_id>")
            return
        ...
        try:
            ...
            if not signal:
                update.message.reply_text(f"❌ Signal {signal_id} not found")
                return
                
            if signal.status != 'ACTIVE':
                update.message.reply_text(f"⚠️ Signal {signal_id} is not active")
                return
            ...
            update.message.reply_text("⚠️ Error cancelling signal")
            session.rollback()
        finally:
            Session.remove()
